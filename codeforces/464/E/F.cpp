...
const int N=310000,Mod=1000000007,Base=1000000009,NN=120000;
int po[N],cnt,n,m,root[N],dis[N],po2[N],al,Ans[N],pre[N],vis[N],flag;
vector<int> E[N],F[N];
struct lj{int c,l,r,len,hash;}t[20000000];
int Chen(int a,int b,int c){return (LL)a*b%(LL)c;}
...
int main()
{
	...
	if (S==T) {printf("0\n1\n%d",S);return 0;}
	po[0]=po2[0]=1;
	rep(i,1,110000) po[i]=Chen(po[i-1],2,Base),po2[i]=Chen(po2[i-1],2,Mod);
	Q.push(S);
	while (Q.size())
	{
